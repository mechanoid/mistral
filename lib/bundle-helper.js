...
module.exports = (config) => ({
  ...
  collectLibs: (bundle) => glob(prefixGlobPattern(config.bundle.base, bundle)),

  modulePath: (libPath) => path.resolve(config.mistral.paths.dist, moduleBasePath(config.bundle.base, libPath), `${libName(libPath)}.js`),

  flattenBundles: (globs) => globs.reduce((bundles, bundle) => bundles.concat(bundle), []),

  externals: (bundles) => bundles.map((libPath) => {
    const moduleBase = moduleBasePath(config.bundle.base, libPath)
